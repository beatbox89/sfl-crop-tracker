...
async function main() {
  ...
  // Ang totoong shape: { collectibles: [...], wearables: [...], updatedAt: "..." }
  let items = null;
  if (Array.isArray(raw?.collectibles) || Array.isArray(raw?.wearables)) {
    items = [...(raw.collectibles || []), ...(raw.wearables || [])];
  ...
  if (!items) {
    // Hindi natin nakilala yung shape — i-print ang structure para makita natin sa logs
